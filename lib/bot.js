...
(function() {
  ...
  Bot = (function() {
    ...
    Bot.prototype.config = function(options) {
      ...
      this.source = typeof this.options.source === 'string' ? path.resolve(this.options.source) : null;
    ...
    Bot.prototype.build = function(callback) {
      if (this.options.enable === false) {
        log.debug(this.output, 'is disabled');
        this.built = true;
        if (callback != null) {
        ...
        log.debug(this.output, 'is being built');
      }
      if (log.level() > 0) {
        _.print("Building " + this.options.output);
      }
    ...
    Bot.prototype.writeContent = function() {
      ...
      if (this.returnContent != null) {
        log.debug("Returning content to callback", path.relative(process.cwd(), this.output));
        ...
        log.debug("Writing", path.relative(process.cwd(), this.output));
