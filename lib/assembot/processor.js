...
(function() {
  var Processor, ProcessorManager, addProcessor, assembot_package, async, build_package, compile_less, compile_stylus, compontent_package, log, manager, packager, path, project_package, project_root, render, replaceTokens, targetOf, tokenParser, transpile, validTarget, _,
    __slice = [].slice;
  ...
  replaceTokens = function(string, context) {
    if (context.replaceTokens && tokenParser.test(string)) {
  ...
  */
  ...
  addProcessor().ext('.json').build(function() {
    return function(source, opts, converted) {
      var data;
  ...
  addProcessor('js').ext('.settee').requires('settee-templates').warn("Settee requires a runtime, be sure it's included in your page.").build(function(settee) {
    return function(source, opts, converted) {
      var output;
